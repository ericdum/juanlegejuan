...
function updateVertexBuffer() {
    const widthRatio = image.width*0.7 / canvas.width;
    const heightRatio = image.height*0.7 / canvas.height;
    const vertices = new Float32Array([
        widthRatio, -heightRatio, 1.0, 1.0,
        widthRatio, heightRatio, 1.0, 0.0,
        -widthRatio, -heightRatio, 0.0, 1.0,
...
function draw() {
    gl.clearColor(134.0, 134.0, 134.0, 0.0);
...
function start(alpha, antialias, useWebgl2) {
    ...
    tick();
    return loadImage('zhonggao.png').then(() => {
        updateVertexBuffer();
        updateTexture();
